...
import { Loader2, Sparkles, ArrowRight, Download, PenTool, Send } from 'lucide-react';
    ...
    >
      ...
      >
        <div className="flex-1">
          <h1 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white font-display tracking-tight">Revisi Otomatis</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium">Perbaiki paragraf skripsi Anda berdasarkan catatan revisi dari dosen.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <motion.div variants={itemVariants} className="space-y-6">
          <div className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-800">
            <form onSubmit={handleRevisi} className="space-y-6">
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Teks Asli (Sebelum Revisi)</label>
                <textarea
                  required
                  value={teksAsli}
                  onChange={(e) => setTeksAsli(e.target.value)}
                  className="w-full h-48 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none resize-none transition-all placeholder:text-slate-400"
                  placeholder="Paste paragraf atau bagian skripsi yang perlu direvisi di sini..."
                />
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Catatan Dosen Pembimbing</label>
                <textarea
                  required
                  value={catatan}
                  onChange={(e) => setCatatan(e.target.value)}
                  className="w-full h-24 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none resize-none transition-all placeholder:text-slate-400"
                  placeholder="Contoh: Tolong perbaiki tata bahasanya, tambahkan referensi teori X..."
                />
              </div>
              <button
                type="submit"
        ...
        <motion.div variants={itemVariants} className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden flex flex-col min-h-[500px]">
          <div className="flex items-center justify-between px-8 py-5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
            <div className="flex items-center gap-3">
              ...
              <h2 className="font-bold text-slate-900 dark:text-white font-display">Hasil Revisi</h2>
            </div>
          </div>
          ...
          <div className="p-8 flex-1 overflow-y-auto relative">
            {loading ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-400 space-y-6 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm z-10">
                ...
                <p className="font-bold animate-pulse">AI sedang memperbaiki teks Anda...</p>
              </div>
            ) : hasil ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-slate-400 py-20 text-center">
                ...
                <p className="font-bold text-lg">Belum ada hasil revisi</p>
                <p className="text-sm mt-1">Hasil revisi akan muncul di sini</p>
              </div>
            )}
          </div>
        </motion.div>
